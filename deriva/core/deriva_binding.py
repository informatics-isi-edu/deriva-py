
import uuid
import sys
import os
import json
import requests
from multiprocessing import Queue
from . import get_new_requests_session, urlquote_dcctx, ConcurrentUpdate, NotModified, DEFAULT_HEADERS, DEFAULT_SESSION_CONFIG
from .utils.sqlite3_utils import SqlConstraint, SqlType, sql_identifier, sql_literal

class DerivaClientContext (dict):
    """Represent Deriva-Client-Context header content.

    Well-known keys (originally defined for Chaise):

    - `cid`: client application ID i.e. program name

    - `wid`: window ID i.e. request stream ID

    - `pid`: page ID i.e. sub-stream ID

    - `action`: UX action embodied by request(s)

    - `table`: table upon which application is focused (if any)

    - `uinit`: True if request initiated by user action

    Default values to use process-wide:

    - `cid`:code:: os.path.basename(sys.argv[0]) if available

    - `wid`:code:: a random UUID

    The process-wide defaults MAY be customized by mutating
    `DerivaClientContext.defaults`:code: prior to constructing instances.

    """
    defaults = {
        'cid': os.path.basename(sys.argv[0]) if len(sys.argv) > 0 and sys.argv[0] else None,
        'wid': uuid.uuid4().hex,
    }

    def __init__(self, *args, **kwargs):
        """Initialize DerivaClientContext from keywords or dict-like.

           Class-wide defaults are used for fields not set explicitly during construction.
        """
        super(DerivaClientContext, self).__init__(*args, **kwargs)
        self.set_defaults()
        self.prune()

    def set_defaults(self, defaults=None):
        """Set default key-values in self if key not already set."""
        if defaults is None:
            defaults = self.defaults
        for k, v in defaults.items():
            if v is not None and (k not in self or self[k] is None):
                self[k] = v

    def prune(self):
        """Prune redundant keys to shorten context representation.

           Keys with None value or uinit=False are unnecessary as these
           are implicitly assumed when absent.

        """
        for k, v in self.items():
            if v is None:
                del self[k]
        if 'uinit' in self and not self['uinit']:
            del self['uinit']

    def encoded(self):
        """Encode self as string suitable for Deriva-Client-Context HTTP header."""
        self.set_defaults()
        self.prune()
        x = urlquote_dcctx(json.dumps(self, indent=None, separators=(',', ':')))
        return x

    def merged(self, overrides):
        c = DerivaClientContext(self)
        c.update(overrides)
        return c

class DerivaPathError (ValueError):
    pass

def _response_raise_for_status(self):
    """Raises requests.HTTPError if status code indicates an error.

    This unbound method can be monkey-patched onto a requests.Response
    instance or manually invoked on one.

    """
    if 400 <= self.status_code < 600:
        details = " Details: %s"
        raise requests.HTTPError(
            u'%s %s Error: %s for url: [%s]%s' % (
                self.status_code,
                'Client' if self.status_code < 500 else 'Server',
                self.reason,
                self.url,
                details % self.content if self.content else "",
            ),
            response=self
        )


class DerivaBinding (object):
    """This is a base-class for implementation purposes. Not useful for clients."""

    def __init__(self, scheme, server, credentials=None, caching=True, session_config=None):
        """Create HTTP(S) server binding.

           Arguments:
             scheme: 'http' or 'https'
             server: server FQDN string
             credentials: credential secrets, e.g. cookie
             caching: whether to retain a GET response cache

           Deriva Client Context: You MAY mutate self.dcctx to
           customize the context for this service endpoint prior to
           invoking web requests.  E.g.:

             self.dcctx['cid'] = 'my application name'

           You MAY also supply custom per-request context by passing a
           headers dict to web request methods, e.g. 

             self.get(..., headers={'deriva-client-context': {'action': 'myapp/function1'}})

           This custom header will be merged as override values with
           the default context in self.dcctx in order to form the
           complete context for the request.

        """
        self._base_server_uri = "%s://%s" % (
            scheme,
            server
        )
        self._server_uri = self._base_server_uri
        self._auth_uri = self._base_server_uri + "/authn/session"

        self._session = None
        self.session_config = DEFAULT_SESSION_CONFIG if not session_config else session_config
        self._get_new_session(self.session_config)

        self._caching = caching
        self._cache = {}

        self._response_raise_for_status = _response_raise_for_status

        self.dcctx = DerivaClientContext()

        self.set_credentials(credentials, server)

    def get_server_uri(self):
        return self._server_uri

    def _get_new_session(self, session_config=None):
        self._close_session()
        self._session = get_new_requests_session(self._server_uri + '/',
                                                 session_config if session_config else self.session_config)

    def _pre_get(self, path, headers):
        self.check_path(path)
        url = self._server_uri + path
        headers = headers.copy()
        prev_response = self._cache.get(url)
        if prev_response and 'etag' in prev_response.headers \
           and not ('if-none-match' in headers or 'if-match' in headers):
            headers['if-none-match'] = prev_response.headers['etag']
        else:
            prev_response = None
        headers['deriva-client-context'] = self.dcctx.merged(headers.get('deriva-client-context', {})).encoded()
        return url, headers, prev_response

    def _pre_mutate(self, path, headers, guard_response=None):
        self.check_path(path)
        url = self._server_uri + path
        headers = headers.copy()
        if guard_response and 'etag' in guard_response.headers:
            headers['If-Match'] = guard_response.headers['etag']
        headers['deriva-client-context'] = self.dcctx.merged(headers.get('deriva-client-context', {})).encoded()
        return url, headers

    @staticmethod
    def check_path(path):
        if not path:
            raise DerivaPathError("Path not specified")

        if not path.startswith("/"):
            raise DerivaPathError("Malformed path error (not rooted with \"/\"): %s" % path)

    @staticmethod
    def _raise_for_status_304(r, p, raise_not_modified):
        if r.status_code == 304:
            if raise_not_modified:
                raise NotModified(p or r)
            else:
                return p or r

        _response_raise_for_status(r)
        setattr(r, 'raise_for_status', _response_raise_for_status.__get__(r))
        return r

    @staticmethod
    def _raise_for_status_412(r):
        if r.status_code == 412:
            raise ConcurrentUpdate(r)
        _response_raise_for_status(r)
        setattr(r, 'raise_for_status', _response_raise_for_status.__get__(r))
        return r

    def set_credentials(self, credentials, server):
        if not credentials:
            return
        assert self._session is not None
        if 'bearer-token' in credentials:
            self._session.headers.update({'Authorization': 'Bearer {token}'.format(token=credentials['bearer-token'])})
        elif 'cookie' in credentials:
            cname, cval = credentials['cookie'].split('=', 1)
            # Fix for cookielib domain rewrite to *.local when no "." in hostname. In this case, don't set the domain.
            # Covers "localhost" and other dev/test scenarios. See "https://github.com/psf/requests/issues/5388"
            self._session.cookies.set(cname, cval, domain="" if "." not in server else server, path='/')
        elif 'username' in credentials and 'password' in credentials:
            self.post_authn_session(credentials)

    def get_authn_session(self):
        headers = { 'deriva-client-context': self.dcctx.encoded() }
        r = self._session.get(self._auth_uri, headers=headers)
        _response_raise_for_status(r)
        return r

    def post_authn_session(self, credentials):
        headers = { 'deriva-client-context': self.dcctx.encoded() }
        r = self._session.post(self._auth_uri, data=credentials, headers=headers)
        _response_raise_for_status(r)
        return r

    def head(self, path, headers=DEFAULT_HEADERS, raise_not_modified=False):
        """Perform HEAD request, returning response object.

           Arguments:
             path: the path within this bound server
             headers: headers to set in request
             raise_not_modified: raise HTTPError for 304 response
               status when true.

           May consult built-in cache and apply 'if-none-match'
           request header unless input headers already include
           'if-none-match' or 'if-match'. On cache hit, returns cached
           response unless raise_not_modified=true. Cached response
           may include content retrieved by GET on the same resource.

        """
        url, headers, prev_response = self._pre_get(path, headers)
        return self._raise_for_status_304(
            self._session.head(url, headers=headers),
            prev_response,
            raise_not_modified
        )
        
    def get(self, path, headers=DEFAULT_HEADERS, raise_not_modified=False, stream=False):
        """Perform GET request, returning response object.

           Arguments:
             path: the path within this bound server
             headers: headers to set in request
             raise_not_modified: raise HTTPError for 304 response
               status when true.
             stream: whether to defer content retrieval to 
               streaming access mode on response object.

           May consult built-in cache and apply 'if-none-match'
           request header unless input headers already include
           'if-none-match' or 'if-match'. On cache hit, returns cached
           response unless raise_not_modified=true.

           Caching of new results is disabled when stream=True.

        """
        if headers is None:
            headers = {}
        url, headers, prev_response = self._pre_get(path, headers)
        r = self._raise_for_status_304(
            self._session.get(url, headers=headers),
            prev_response,
            raise_not_modified
        )
        if self._caching and not stream:
            self._cache[url] = r
        return r

    def post(self, path, data=None, json=None, headers=DEFAULT_HEADERS):
        """Perform POST request, returning response object.

           Arguments:
             path: the path within this bound server
             data: a buffer or file-like content value
             json: data to serialize as JSON content
             headers: headers to set in request

           Raises ConcurrentUpdate for 412 status.

        """
        url, headers = self._pre_mutate(path, headers)
        r = self._session.post(url, data=data, json=json, headers=headers)
        return self._raise_for_status_412(r)

    def put(self, path, data=None, json=None, headers=DEFAULT_HEADERS, guard_response=None):
        """Perform PUT request, returning response object.

           Arguments:
             path: the path within this bound server
             data: a buffer or file-like content value
             json: data to serialize as JSON content
             headers: headers to set in request
             guard_response: expected current resource state
               as previously seen response object.

           Uses guard_response to build appropriate 'if-match' header
           to assure change is only applied to expected state.

           Raises ConcurrentUpdate for 412 status.

        """ 
        url, headers = self._pre_mutate(path, headers, guard_response)
        r = self._session.put(url, data=data, json=json, headers=headers)
        return self._raise_for_status_412(r)
   
    def delete(self, path, headers=DEFAULT_HEADERS, guard_response=None):
        """Perform DELETE request, returning response object.

           Arguments:
             path: the path within this bound server
             headers: headers to set in request
             guard_response: expected current resource state
               as previously seen response object.

           Uses guard_response to build appropriate 'if-match' header
           to assure change is only applied to expected state.

           Raises ConcurrentUpdate for 412 status.

        """
        url, headers = self._pre_mutate(path, headers, guard_response)
        r = self._session.delete(url, headers=headers)
        return self._raise_for_status_412(r)

    def _close_session(self):
        if self._session is not None:
            self._session.close()
            self._session = None

    def __del__(self):
        self._close_session()


class AbstractObserver:
    """Extensible base class for polling observers.

    Fundamental assumptions:

    1. A remote tabular source exists with a schema which can be
       mapped to a local Sqlite3 table schema.

    2. A paginated, temporally ordered retrieval protocol is supported
       to reliably fetch remote records in order of last
       modification. The last modification time exists in a field of
       each record and behaves coherently and monotonically to the
       remote observer. A record can never appear to change to an
       "earlier" last modified time compared to any last modified time
       that has been observed by a prior retrieval operation.

    3. There is a single column, sortable primary key. Thus, a
       composite sort of (last modified time, primary key) provides a
       total order of all records available in any single polling
       request.

    Theory of operation:

    1. Two sqlite3 tables will be provisioned. One is the observer's
       replica of remote tabular state. The other is a temporary
       "inbound" buffer, holding recently retrieved pages.

    2. There is a logical time cursor representing the most recent
       changes the observer has observed.

    3. The fetch operation retrieves all available records which have
       been more recently changes since the last observation. These
       are loaded into the inbound buffer table.

    4. The merge operation upserts the inbound buffer content into the
       replica table.

    The last observed time cursor is the latest time among both the
    replica and inbound tables, describing the lower bound for any
    future fetch records.

    The split fetch and merge phases allows for more strict integrity
    constraints to be applied to a replica table, if desired. During
    fetch, the inbound table might be incoherent due to pagination and
    the temporal ordering. E.g. in a table with self-referencing
    foreign keys, a dependent row might be retrieved in a page prior
    to a referenced row which has a more recent last modification
    time.

    """

    # these must be provisioned by subclasses
    column_defs = [
        # (name, type, flags, refconstr?)
    ]
    table_constraints = ""
    sort_cnames = None
    sort_pos0 = None
    pagesize = 1000

    def __init__(self, base_tname):
        """Initialize a polling observer.

        :param base_tname: The name of the replica table in sqlite3.

        A secondary inbound table name will be generated by adding the
        suffix "_inbound" to the base_tname.

        """
        self.tname = base_tname
        self.tname_inbound = f"{base_tname}_inbound"
        # sanity check
        columns = { cdef[0]: cdef for cdef in self.column_defs }
        for cname in self.sort_cnames:
            if cname not in columns:
                raise ValueError(f'sort {cname=} not in column_defs!')
        cname = self.sort_cnames[-1]
        if not columns[cname][2] & SqlConstraint.PRIMARYKEY:
            raise ValueError(f'final sort {cname=} must be PRIMARY KEY!')

    def sqlite3_provision(self, conn):
        """Perform idempotent DDL setup on database.

        :param conn: The sqlite3 DB connection for executing statements.
        """
        with conn:
            cur = conn.cursor()
            cur.execute(self._table_ddl(self.tname, True))
            cur.execute(self._table_ddl(self.tname_inbound, False))

    def _col_ddl(self, cdef, show_constr=True):
        """Return SQL DDL fragment for column definition.

        :param cdef: The column definition tuple (cname, typestr, flags, col_constr?).
        :param show_constr: True when col_constr should appear in result.
        """
        sqlname = sql_identifier(cdef[0])
        sqltype = cdef[1]
        flags = ' '.join([
            flag
            for flag, bitv in [
                ("PRIMARY KEY", SqlConstraint.PRIMARYKEY),
                ("UNIQUE", SqlConstraint.UNIQUE),
                ("NOT NULL", SqlConstraint.NOTNULL),
            ]
            if cdef[2] & bitv
        ])
        constr = cdef[3] if show_constr and len(cdef) > 3 else ""
        return f"{sqlname} {sqltype} {flags} {constr}"

    def _table_ddl(self, tname, show_constr=True):
        """Return SQL DDL statement for table definition.

        :param dtups: The column definition tuples (cname, typestr, flags, col_constr?).
        :param show_constr: True when col_constr should appear in result.
        :param table_constraints: Any table constraint(s) to append to definition.
        """
        sqlcols = ',\n  '.join([
            self._col_ddl(cdef, show_constr=True)
            for cdef in self.column_defs
        ])
        table_constraints = f", {self.table_constraints}" if self.table_constraints else ""
        return f"""
CREATE TABLE IF NOT EXISTS {sql_identifier(tname)} (
  {sqlcols}{table_constraints}
) WITHOUT ROWID;"""

    def _get_page(self, *pos):
        """Return the page following the given position.

        :param pos: The tuple of page boundary keys
        """
        raise NotImplementedError('_get_page on abstract base AbstractObserver')

    def get_position(self, cur):
        """Get and return last_id, last_modified_at position.
        """
        pos = self.sort_pos0

        cols = ", ".join([ sql_identifier(cname) for cname in self.sort_cnames ])
        sort = ", ".join([ f"{sql_identifier(cname)} DESC" for cname in self.sort_cnames ])
        for tname in [self.tname, self.tname_inbound]:
            query =  f"SELECT {cols} FROM {tname} ORDER BY {sort} LIMIT 1"
            cur.execute(query)
            pos = next(cur, pos)
        return pos

    def get_counts(self, conn):
        """Return integer pair of counts for (replicated,inbound) records."""
        cnts = []
        with conn:
            cur = conn.cursor()
            for tname in [self.tname, self.tname_inbound]:
                cur.execute(f"SELECT count(*) FROM {tname}")
                cnts.append(next(cur)[0])
        return cnts

    def print_stats(self, conn):
        """Print observer statistics.
        """
        n1, n2 = self.get_counts(conn)
        print(f"{self.tname}: {n1} records ({n2} inbound)")

    def fetch_inbound(self, conn):
        """Fetch all available record updates and insert into inbound table.
        
        :param conn: The sqlite3 DB connection for the inbound table.
        """
        # get current polling observer position in stream
        with conn:
            cur = conn.cursor()
            pos = self.get_position(cur)
            cur = None

        # fetch all available records until exhaustion
        while True:
            rows = self._get_page(*pos)

            if not rows:
                break
            else:
                pos = [ rows[-1][cname] for cname in self.sort_cnames ]

            # checkpoint each page under a separate transaction
            insertcols = ", ".join([
                sql_identifier(cdef[0])
                for cdef in self.column_defs
            ])
            valparams = ", ".join([
                ("json(?)" if cdef[1] == "JSON" else "?")
                for cdef in self.column_defs
            ])
            setexprs = ", ".join([
                f"{sql_identifier(cname)} = EXCLUDED.{sql_identifier(cname)}"
                for cname, *rest in self.column_defs
                if cname != self.sort_cnames[-1]
            ])
            pkey = sql_identifier(self.sort_cnames[-1])
            with conn:
                cur = conn.cursor()
                for row in rows:
                    cur.execute(f"""
INSERT INTO {self.tname_inbound} ({insertcols})
VALUES({valparams})
ON CONFLICT ({pkey})
DO UPDATE SET {setexprs};
                    """,
                    [ row[k] for k, *rest in self.column_defs ],
                                )

            if len(rows) < self.pagesize:
                break

    def merge_inbound(self, conn):
        """Merge inbound records into replica table, yielding (name, merged) stats.

        :param conn: The sqlite3 DB connection for the inbound and replica tables.

        """
        insertcols = ", ".join([
            sql_identifier(cdef[0])
            for cdef in self.column_defs
        ])
        setexprs = ", ".join([
            f"{sql_identifier(cname)} = EXCLUDED.{sql_identifier(cname)}"
            for cname, *rest in self.column_defs
            if cname != self.sort_cnames[-1]
        ])
        pkey = sql_identifier(self.sort_cnames[-1])
        with conn:
            cur = conn.cursor()
            cur.execute(f"""
INSERT INTO {sql_identifier(self.tname)} ({insertcols})
SELECT {insertcols}
FROM {self.tname_inbound}
WHERE True
ON CONFLICT ({pkey})
DO UPDATE SET {setexprs};
            """)
            cur.execute(f"SELECT count(*) FROM {self.tname_inbound};")
            n = next(cur)[0]
            cur.execute(f"DELETE FROM {self.tname_inbound};")
            yield (self.tname, n)

    def sync(self, conn):
        """Perform fetch_inbound and merge_inbound steps, yieldng (name, merged) stats.

        :param conn: The sqlite3 DB connection for the inbound and replica tables.
        """
        self.fetch_inbound(conn)
        for v in self.merge_inbound(conn):
            yield v

class CompositeObserver:
    """Client-side replica of DERIVA managed asset info.

    Each instance represents a stateful observer managing a replica of
    multiple observed tables.

    """
    def __init__(self, *observers):
        """Initialize new replica

        :param observers: A sequence of observers to compose

        The observers MUST be passed in topologically sorted order
        such that:

        1. Observer tables can be provisioned in forward order.
        2. Observer inbound can be fetched in reverse order.
        3. Observer inbound can be merged in forward order.

        """
        self.observers = list(observers)
        self.observers_reversed = list(self.observers)
        self.observers_reversed.reverse()

    def sqlite3_provision(self, conn):
        """Perform idempotent DDL setup on database.
        
        :param conn: The sqlite3 DB connection for the inbound and replica tables.
        """
        for observer in self.observers:
            observer.sqlite3_provision(conn)

    def fetch_inbound(self, conn):
        """Fetch all available record updates and insert into inbound table.
        
        :param conn: The sqlite3 DB connection for the inbound table.
        """
        for observer in self.observers_reversed:
            observer.fetch_inbound(conn)

    def merge_inbound(self, conn):
        """Merge inbound records into replica table, yielding (name, merged) stats.

        :param conn: The sqlite3 DB connection for the inbound and replica tables.
        """
        for observer in self.observers:
            for v in observer.merge_inbound(conn):
                yield v

    def sync(self, conn):
        """Perform fetch_inbound and merge_inbound steps, yielding (name, merged) stats.

        :param conn: The sqlite3 DB connection for the inbound and replica tables.
        """
        self.fetch_inbound(conn)
        for v in self.merge_inbound(conn):
            yield v

    def print_stats(self, conn):
        """Print observer statistics.
        """
        for observer in self.observers:
            observer.print_stats(conn)
